...
+ (BOOL)fallsToday
{
    return [KCSefiratHaomer fallsOnDate:[NSDate date]];
}

//  YES if date is during the omer, else NO
+ (BOOL)fallsOnDate:(NSDate *)date
{
    return [KCSefiratHaomer dayOfSefiraForDate:date] != 0;
}
...
{
	return [KCSefiratHaomer dayOfSefiraForDate:[NSDate date]];
}

// Returns 1-49, or 0 for all other days
+ (NSInteger)dayOfSefiraForDate:(NSDate *)date
{
	NSDate *firstDayOfTheOmer = [KCSefiratHaomer _dateOfSixteenNissanForYearOfDate:date];
